...
static void save_quality_result(VP9_COMMON *cm)
{
    ...
    if(vpx_deserialize_load(cm->reference_frame_hr, file_path, width_, height_,
    ...
    double weight;
    double ssim = vpx_calc_ssim(get_frame_new_buffer(cm), cm->reference_frame_hr, &weight);
    vpx_calc_psnr(get_frame_new_buffer(cm), cm->reference_frame_hr, &psnr);
...
static void debug_lr_quality(VP9_COMMON *cm)
{
    char file_path[PATH_MAX];
    PSNR_STATS psnr;
    memset(file_path, 0, sizeof(char) * PATH_MAX);
    ...
    sprintf(file_path, "%s/%d_%s.serialize", cm->decode_info->serialize_dir, cm->current_video_frame - 1, cm->decode_info->target_file);
    if(vpx_deserialize_load(cm->reference_frame_lr, file_path, width_, height_,
                            cm->subsampling_x, cm->subsampling_y, cm->byte_alignment))
    {
        vpx_internal_error(&cm->error, VPX_MOBINAS_ERROR,
                           "deserialize failed");
    }
    vpx_calc_psnr(get_frame_new_buffer_lr(cm), cm->reference_frame_lr, &psnr);

    LOGD("Low-resolution quality, %d, %.2fdB", cm->current_video_frame - 1, psnr.psnr[0]);
}
...
static vpx_codec_err_t decoder_decode(vpx_codec_alg_priv_t *ctx,
    ...
    cm->decode_info = (decode_info_t *) user_priv;
    ...
    if (frame_count > 0) {
        ...
        while (data_start < data_end) {
            ...
            /*******************Hyunho************************/
            if (cm->decode_info->save_intermediate) {
                if (cm->decode_info->save_serialized_frame) save_serialized_intermediate_frame(cm,
                                                                                               cm->current_video_frame -
                                                                                               1,
                                                                                               cm->current_super_frame);
                if (cm->decode_info->save_decoded_frame) save_decoded_intermediate_frame(cm,
                                                                                         cm->current_video_frame -
                                                                                         1,
                                                                                         cm->current_super_frame);
            }
    ...
    if (cm->decode_info->debug_quality) debug_lr_quality(cm);
    /*******************Hyunho************************/
