    ...
    def enforce_input_type(cls, graph, input_name, target_format, permute_order):
        ...
        if input_buf.axis_format == cls.AxisFormat.NONTRIVIAL:
            if input_buf.rank() == len(permute_order):
                graph.inject_implicit_permute(input_name, target_format, permute_order)
            else:
...
class AxisOrder(object):
    def __init__(self):
        ...
        # This is the permute order needed to convert axes to SNPE order from self order
...
class OnnxAxisOrder(AxisOrder):
    ...
    @classmethod
    def format_time_series_output_shape(cls, batch_size, time_steps, feature):
        return [time_steps, 1, batch_size, feature]
