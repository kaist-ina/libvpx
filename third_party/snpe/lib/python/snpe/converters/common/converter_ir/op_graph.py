...
class OpNode(object):
    def __init__(self, op, input_names, output_names, axis_order):
        self.op = op
...
class IROpGraph(object):
    ...
    def add_quantization_params(self, op_name, **kwargs):
        ...
        output_encodings.extend(snpe_translation_utils.to_list(kwargs.get(QuantParams.OUTPUT_ENCODINGS, [])))
        param_encodings.extend(snpe_translation_utils.to_list(kwargs.get(QuantParams.PARAM_ENCODINGS, [])))
    ...
    def __insert_node(self, node, output_shapes, axis_formats=None, idx=-1):
        ...
        for i, (name, shape) in enumerate(zip(node.output_names, output_shapes)):
            ...
            output_buffers.append(self.buffers[name])
    ...
    def add(self, op, input_names, output_names, axis_formats=None):
        """
        ...
        self.__insert_node(node, output_shapes, axis_formats)
    ...
    def add_input(self, name, shape, axis_format=None):
