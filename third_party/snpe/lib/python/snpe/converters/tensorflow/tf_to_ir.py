...
class TFConverter(ConverterBase):
    class ArgParser(ConverterBase.ArgParser):
        def __init__(self):
            super(TFConverter.ArgParser, self).__init__("tensorflow")
            ...
            self.parser.add_required_argument('-d', '--input_dim', nargs=2, action='append',
                                              ...
                                              help="The names and dimensions of the network input layers specified in"
                                                   "the format [input_name comma-separated-dimensions], "
                                                   "for example: 'data' 1,224,224,3. Note that the quotes should always"
                                                   "be included in order to handle special characters, spaces, etc. "
    ...
    def __init__(self, session, args):
        ...
        self._strict_node_resolution = not args.allow_unconsumed_nodes
        ...
        loader = ModelLoader(session, input_model_path, in_nodes,
                             in_dims, in_types, args.out_node)
    ...
    def _convert_input_layers(self):
        ...
        for model_input in self._context.inputs:
            ...
            self.graph.add_input(layer_name, shape)
