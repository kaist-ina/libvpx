...
# =============================================================================
import numpy as np
from snpe.converters.common.converter_ir.op_adapter import (
    ReduceMaxOp,
    ReduceMeanOp,
    ReduceMinOp,
    ReduceProdOp,
...
class ReductionMeanLayerBuilder(ReductionLayerBuilder):
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        return ir_graph.add(ReduceMeanOp(name=descriptor.layer_name,
...
class ReductionProdLayerBuilder(ReductionLayerBuilder):
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        input_name = self.get_input_name(converter_context, descriptor, input_descriptors)
        output_name = descriptor.output_names[0]
        return ir_graph.add(ReduceProdOp(name=descriptor.layer_name,
                                         axes=descriptor.axes,
                                         keep_dims=descriptor.keep_dims),
                            input_names=[input_name],
                            output_names=[output_name])
...
class ReductionSumLayerBuilder(ReductionLayerBuilder):
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        input_name = self.get_input_name(converter_context, descriptor, input_descriptors)
        output_name = descriptor.output_names[0]
        return ir_graph.add(ReduceSumOp(name=descriptor.layer_name,
                                        axes=descriptor.axes,
                                        keep_dims=descriptor.keep_dims),
                            input_names=[input_name],
                            output_names=[output_name])
...
class ReductionMinLayerBuilder(ReductionLayerBuilder):
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        input_name = self.get_input_name(converter_context, descriptor, input_descriptors)
        output_name = descriptor.output_names[0]
        return ir_graph.add(ReduceMinOp(name=descriptor.layer_name,
                                        axes=descriptor.axes,
                                        keep_dims=descriptor.keep_dims),
                            input_names=[input_name],
                            output_names=[output_name])
...
class ReductionMaxLayerBuilder(ReductionLayerBuilder):
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        input_name = self.get_input_name(converter_context, descriptor, input_descriptors)
        output_name = descriptor.output_names[0]
        return ir_graph.add(ReduceMaxOp(name=descriptor.layer_name,
                                        axes=descriptor.axes,
                                        keep_dims=descriptor.keep_dims),
                            input_names=[input_name],
                            output_names=[output_name])
