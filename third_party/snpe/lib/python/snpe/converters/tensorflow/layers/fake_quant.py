    ...
    def resolve_layer(self, graph_matcher, graph_helper):
        ...
        for sequence in self.sequences:
            ...
            for match in matches:
                ...
                is_act_quant = False if input_op.type in ['Identity', 'Const'] or \
                                        graph_helper.check_op_const_origin(input_op) else True
                min = self._get_float(graph_helper, min_op)
                max = self._get_float(graph_helper, max_op)
                bw = self._get_float(graph_helper, matches["num_bits"]) if "num_bits" in matches else 8
    ...
    def transform_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        if len(output_descriptors) != 0:
            if descriptor.is_act_quant or \
               len([d for d in input_descriptors if isinstance(d, BatchNormLayerResolver.Descriptor) and d.bn_folded]) == 1:
