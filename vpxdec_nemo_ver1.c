...
{   &help, &codecarg, &use_yv12, &use_i420, &flipuvarg, &rawvideo, &noblitarg, &progressarg, &limitarg, &skiparg,
    ...
    &outputwidtharg, &outputheightarg, &decodemodearg, &dnnmodearg, &cachemodearg,
...
static void setup_directory(nemo_cfg_t *nemo_cfg, const char *data_dir, const char *content, int input_resolution, int reference_resolution,
    ...
    if (nemo_cfg->decode_mode == DECODE) {
        ...
        sprintf(nemo_cfg->log_dir, "%s/%s/log/%s", data_dir, content, input_video_name);
        sprintf(nemo_cfg->input_frame_dir, "%s/%s/image/%s", data_dir, content, input_video_name);

        if (nemo_cfg->save_quality) {
            reference_video_name = get_video_name(reference_resolution);
            sprintf(nemo_cfg->input_reference_frame_dir, "%s/%s/image/%s", data_dir, content, reference_video_name);
        }

        if (postfix != NULL) {
            sprintf(nemo_cfg->log_dir, "%s/%s", nemo_cfg->log_dir, postfix);
        }
    }
    else if (nemo_cfg->decode_mode == DECODE_SR || nemo_cfg->decode_mode == DECODE_CACHE) {
        input_video_name = get_video_name(input_resolution);
        dnn_name = get_dnn_name(input_resolution, quality);
        sprintf(nemo_cfg->log_dir, "%s/%s/log/%s/%s", data_dir, content, input_video_name, dnn_name);
        sprintf(nemo_cfg->input_frame_dir, "%s/%s/image/%s", data_dir, content, input_video_name);
        sprintf(nemo_cfg->sr_frame_dir, "%s/%s/image/%s/%s", data_dir, content, input_video_name, dnn_name);
        sprintf(nemo_cfg->sr_offline_frame_dir, "%s/%s/image/%s/%s", data_dir, content, input_video_name, dnn_name);

        if (nemo_cfg->save_quality) {
            reference_video_name = get_video_name(reference_resolution);
            sprintf(nemo_cfg->input_reference_frame_dir, "%s/%s/image/%s", data_dir, content, input_video_name);
            sprintf(nemo_cfg->sr_reference_frame_dir, "%s/%s/image/%s", data_dir, content, reference_video_name);
        }
