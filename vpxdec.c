...
/* Mobinas input arguments*/
static const arg_def_t contentdirarg =
        ARG_DEF(NULL, "content-dir", 1, "Directory for saving files");
static const arg_def_t inputvideoarg =
        ARG_DEF(NULL, "input-video", 1, "Input video file");
static const arg_def_t dnnvideoarg =
        ARG_DEF(NULL, "dnn-video", 1, "HR Video file used for apply a offline DNN");
static const arg_def_t comparevideoarg =
        ARG_DEF(NULL, "compare-video", 1,
  ...
  vpx_image_t *img_shifted = NULL;
    ...
    for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step)
    {
      ...
      ec_enabled = 1;
        ...
        else if (arg_match(&arg, &cachepolicyarg, argi))
        {
            switch (arg_parse_uint(&arg))
            {
            case PROFILE_CACHE:
